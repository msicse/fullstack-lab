...
const promos = [
  { code: "ostad10", percentage: 10 },
  { code: "ostad50", percentage: 50 },
];
            ...
            <div class="flex-grow">
                ...
                <div class="flex items-center space-x-2 mt-1">
                    <button onclick="updateQuantity(${
                      item.id
                    }, -1)" class="bg-gray-200 px-2 py-0.5 rounded text-xs">${
        item.quantity > 1 ? "-" : '<i class="fa-solid fa-trash-can"></i>'
      }</button>
...
document.getElementById("apply-promo").addEventListener("click", () => {
  ...
  const promo = promos.find((c) => c.code === promoInput.value);

  if (promo) {
    discount = promo.percentage;
    promoMessage.textContent = `promo "${promo.code}" applied!`;
